  ...
  constructor(public navCtrl: NavController,
    ...
    if (changeUsername) {
      ...
      if (isTouchIDEnabledStr == "true") {
        ...
        this.alerts = [this.prepareAlertModal(ConstantsService.ERROR_MESSAGES.LOGINPAGE_REMEMBER_DISABLE, ConstantsService.ERROR_MESSAGES.LOGINPAGE_REENABLE, ConstantsService.ERROR_MESSAGES.LOGINPAGE_ERRORHEADER_SMALL)];
    ...
    this.loginForm = this.fb.group({
      userName: [savedUserid, Validators.compose([Validators.required])],
    ...
    setTimeout(() => {
      ...
      this.loginService.loginRequest(request)
        ...
        .subscribe(response => {
          ...
          if (err.result == "-1" || err.result === "-4" || err.result === "-5" || err.result === "-6" || err.result === "-7" || err.result === "-8" || err.result === "-9"){
            let errorMessage = ConstantsService.ERROR_MESSAGES["MEMBERLOGIN"] && ConstantsService.ERROR_MESSAGES["MEMBERLOGIN"][err.result] ? ConstantsService.ERROR_MESSAGES["MEMBERLOGIN"][err.result] : ConstantsService.ERROR_MESSAGES.LOGINPAGE_ERRORBODY;
